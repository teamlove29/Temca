...
class ReportInThreePhaseActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_report_in_three_phase)
        ...
        if(DataFromWireSize[0].condutiSize.indexOf('/') != -1){
            for(i in 0..2){
                if(DataFromWireSize[0].condutiSize.indexOf(fractionValues[i]) != -1){
                    textViewResultConduitSize.text = DataFromWireSize[0].condutiSize.replace(
                        fractionValues[i],
                        fractionValues2[i])
                    break
            ...
            .withListener(object : PermissionListener {
                override fun onPermissionGranted(p0: PermissionGrantedResponse?) {
                    if (DataFromWireSize.size > 0 ) {
                        createPDFFile(
                            Common.getAppPath(this@ReportInThreePhaseActivity) + file_name, DataFromWireSize
                        )
                    }
                }
                override fun onPermissionDenied(p0: PermissionDeniedResponse?) {
                    showSettingsDialog()
                    Toast.makeText(
                        this@ReportInThreePhaseActivity,
                        "Denied Permission",
                        Toast.LENGTH_LONG
                    ).show()
                }
...
//            document.isMarginMirroring = true
            ...
            addNewItemWithLeftAndRight(document, "รายงานการคำนวนขนาดวงจรไฟฟ้า 3 เฟส", "", titleStyle, detailStyleTitle)
    ...
    fun addNewItemWithLeftAndRight(
        ...
        var chunkTextRight = if (textLeft == "รายงานการคำนวนขนาดวงจรไฟฟ้า 3 เฟส"){
